...
function roihin_crystal_validate_before_save() {
    ...
    if (get_post_type($_POST['post_ID']) !== 'crystal') {
        return;
    }
    ...
    foreach ($required_text_fields as $field => $error_message) {
        if (empty($_POST['acf'][$field]) || trim($_POST['acf'][$field]) === '') {
            $errors[] = $error_message;
        }
    }

    // Validate English name format (alphanumeric + spaces)
    if (!empty($_POST['acf']['crystal_name_en'])) {
    ...
    if (!empty($_POST['acf']['crystal_name_th']) && strlen($_POST['acf']['crystal_name_th']) > 200) {
        $errors[] = __('Thai name cannot exceed 200 characters', 'roihin-crystal');
    }

    // Validate attributes length
    if (!empty($_POST['acf']['crystal_attributes']) && strlen($_POST['acf']['crystal_attributes']) > 2000) {
    ...
    foreach ($required_array_fields as $field => $error_message) {
        if (empty($_POST['acf'][$field]) || !is_array($_POST['acf'][$field]) || count($_POST['acf'][$field]) === 0) {
            $errors[] = $error_message;
        }
    }

    // Validate ruling planet
    if (empty($_POST['acf']['ruling_planet'])) {
        $errors[] = __('Ruling Planet must be selected', 'roihin-crystal');
    }

    // Validate description paragraphs
    if (empty($_POST['acf']['description_paragraphs']) || count($_POST['acf']['description_paragraphs']) === 0) {
        ...
        foreach ($_POST['acf']['description_paragraphs'] as $index => $paragraph) {
            if (empty($paragraph['paragraph_text']) || trim($paragraph['paragraph_text']) === '') {
    ...
    // Validate main image
    if (empty($_POST['acf']['crystal_main_image'])) {
        $errors[] = __('Main Image is required', 'roihin-crystal');
    }

    // Validate slug uniqueness
    if (!empty($_POST['acf']['crystal_slug'])) {
        $slug = sanitize_title($_POST['acf']['crystal_slug']);
        $existing = roihin_crystal_check_slug_exists($slug, $_POST['post_ID']);
        if ($existing) {
            $errors[] = sprintf(
...
function roihin_crystal_set_featured_image($post_id) {
    ...
    if ($main_image && isset($main_image['ID'])) {
        set_post_thumbnail($post_id, $main_image['ID']);
    }
}
